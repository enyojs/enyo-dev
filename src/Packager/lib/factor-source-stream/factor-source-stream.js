...
function FactorSourceStream (opts) {
	...
	opts = opts || {};
	log = logger(opts).child({component: 'factor-source-stream'});
	this.options = opts;
	this.modules = [];
	this.bundles = {};
	this.graph = {};
...
proto.getBundleNameFor = function (entry) {
	...
	else bn = path.basename(opts.name);
	entry.bundleName = bn;
...
proto.trace = function (entry) {
	var db, stream, requests, requested, requires, required, o_required, o_requested, opts, trace, willIgnore;
	stream = this;
	opts = this.options;
	db = log.debug();
	trace = entry.trace = {};
	...
	willIgnore = (opts.skip && entry.libName && opts.skip.indexOf(entry.libName) > -1) || (opts.externals === false && entry.external);
	...
	if (willIgnore) {
		if (db) log.debug({module: entry.relName}, 'module will be ignored because its library has been explicitly skipped or external and externals are set to off');
		entry.bundleName = stream.getBundleNameFor(entry);
		trace.required = true;
		return;
	}
...
proto.customNameFor = function (entry) {
	...
	return util.format('%s+%s', opts.name.replace(/\ /g, '+'), rel);
};
