...
function BundleOutputStream (opts) {
	...
	opts = opts || {};
	log = logger(opts).child({component: 'bundle-output-stream'});
	this.options = opts;
	this.bundles = [];
	this.files = [];
	this.scripts = [];
...
proto._transform = function (bundle, nil, next) {
	log.debug(`Adding bundle "${bundle.name}"`);
...
proto.prepareIndex = Promise.method(function (contents) {
	...
	try {
		...
		console.log(e);
		console.log('FAILED HERE', opts.outDir, opts.outFile, opts)
	}
	this.files.push(ctx);
});
	...
	bundles.forEach(function (bundle) {
		...
		if (bundle.style && !bundle.ignore) {
			if (opts.devMode || opts.library || bundle.request) {
				if (opts.styleOnly && opts.library && opts.outCssFile) fname = opts.outCssFile;
				else fname = bundle.name + '.css';
				file = {
					href: fname,
					outfile: path.join(opts.outDir, fname),
