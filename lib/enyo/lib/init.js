...
function initGitIgnore (opts) {
	if (opts.gitIgnore !== false) {
		return Promise.join(
			opts.env.get('libDir'),
			opts.env.get('outdir'),
			opts.env.get('moduleDir'),
			function (libDir, outdir, moduleDir) {
				return Promise.all([
					opts.env.gitignore.set([libDir, outdir], true),
					opts.env.gitignore.remove([moduleDir])
				]).then(function () {
					return opts.env.gitignore.commit();
				});
			}
		);
	} else return Promise.resolve();
