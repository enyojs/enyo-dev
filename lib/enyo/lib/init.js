...
module.exports = function (opts) {
	return initPackage(opts).then(function () {
		...
		if (opts.library) return link({env: opts.env});
	});
};
...
function updateConfig (opts) {
	if (opts.config !== false) {
		...
		if (opts.reset) {
			...
			else opts.env.config.json = clone(opts.env.user.json, true);
		} else {
			if (opts.library) opts.env.config.json = merge(true, opts.env.system.library, opts.env.config.json || {});
			else opts.env.config.json = merge(true, opts.env.user.json, opts.env.config.json || {});
		}
