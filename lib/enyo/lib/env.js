...
function ignoreSet (adds) {
	...
	if (len < this.lines.length) return this.commit();
	else return resolve();
}
...
function Env (opts) {
	...
	this.gitignore = {set: ignoreSet, commit: ignoreCommit, update: ignoreUpdate.bind(enyo)};
...
proto.validateLocalConfig = Promise.method(function () {
	var json, valid, repl, sources, link, enyo;
	...
	// special case #2: to keep the value of this dynamic with changes we hijack the request
		...
		return this.get('scriptSafe').then(function (scriptSafe) {
			if (scriptSafe) return false;
				...
				if (ret === undefined && !enyo.isProjectConfigProperty(prop)) ret = enyo.user.get(parts);
				return ret;
			}
		});
	} else {
		...
		if (ret === undefined && !this.isProjectConfigProperty(prop)) ret = this.user.get(parts);
	}
