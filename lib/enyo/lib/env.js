...
function set (parts, value, remove) {
	...
	for (i = 0; i < parts.length; ++i) {
		...
		if (i + 1 === parts.length) {
			...
			else if (base[part] && typeof base[part] == 'object' && !remove) {
...
proto.set = function (prop, value, global, array, remove) {
	...
	if (!this.validateProperty(part, remove)) return reject(ERROR.SET_OBJECT, parts.join('.'));
...
proto.validateProperty = function (prop, remove) {
	return this.propertyTypeFor(prop) == 'object' && !remove ? false : true;
};
