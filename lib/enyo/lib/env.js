...
var
	...
	u_templates = path.join(u_dir, 'templates'),
...
var
	...
	// we require this for consistent fatal exception handling
...
function ignoreSet (adds, nocommit) {
	...
	for (i = 0; i < adds.length; ++i) {
		if (adds[i] && this.lines.indexOf(adds[i]) === -1) {
...
function copyConfig (library) {
	...
	return this.get('scriptSafe').then(function (scriptSafe) {
		return scriptSafe ? resolve() : enyo.config.commit();
	});
}
...
function Env (opts) {
	...
	this.userTemplates = u_templates;
	this.system = {};
	...
	this.projects = {get: configGet, set: configSet, commit: configCommit, name: 'user.projects', file: u_projects};
	this.initSystemDefaults().then(function () {
		...
		return enyo.get('scriptSafe').then(function (scriptSafe) {
			return !scriptSafe ? enyo.ensureUserConfig() : enyo.loadUserConfig();
		});
	}).then(function () {
		return enyo.get('scriptSafe').then(function (scriptSafe) {
			return !scriptSafe ? enyo.ensureUserTemplates() : enyo.loadDefaultTemplates();
		});
	}).then(function () {
...
proto.initSystemDefaults = function () {
	...
	steps = [
		...
		fs.readJsonAsync(pkg).then(function (json) {
			enyo.system.package = enyo.package.defaults = json;
		}),
...
proto.ensureUserConfig = function () {
	...
	steps = [
		fs.ensureDirAsync(u_dir).then(function () {
			return enyo.loadUserConfig().then(function () {
				if (!enyo.user.hadConfig) {
					enyo.user.json = clone(enyo.system.config, true);
					return enyo.user.commit();
				}
			}).then(function () {
				if (!enyo.user.hadDefaults) {
		...
		fs.statAsync(u_projects).then(function (stat) {
			if (stat.isDirectory()) {
				// older system, need to upgrade (this was never used but created in previous
				// versions of the tools
				return fs.removeAsync(u_projects);
			}
		}, function () {}).then(function () {
			return fs.ensureFileAsync(u_projects);
		})
	];
	return Promise.all(steps);
};

proto.loadUserConfig = function () {
	var enyo, steps;
	enyo = this;
	steps = [
		...
		fs.readJsonAsync(u_projects).then(function (json) {
			enyo.projects.json = json;
		}, function () {
			enyo.projects.json = {};
		})
	];
...
proto.loadLocals = function () {
	...
	enyo = this;
	opts = this.options;
	p_file = this.package.file = path.resolve(this.cwd, 'package.json');
	config = this.config.file = path.resolve(this.cwd, opts.configFile || '.enyoconfig');
	ignore = this.gitignore.file = path.resolve(this.cwd, '.gitignore');
	return fs.ensureDirAsync(this.cwd).then(function () {
		steps = [
			...
			fs.readJsonAsync(config).then(function (json) {
				...
				return fs.statAsync(config).then(function (stat) {
					if (stat.isFile()) {
						// then it failed to read/parse the JSON so we should let them know
						cli(('Unable to parse the configuration file %s, please check it for errors ' +
							'before proceeding.'), path.relative(enyo.cwd, config));
						process.exit(-1);
					} else {
						cli('The provided configuration file %s is not actually a file.', path.relative(enyo.cwd, config));
						process.exit(-1);
					}
				}, function () {});
			}),
...
proto.set = function (prop, value, global, array, remove) {
	...
	if (!this.hasConfig() && !global && !this.isPackageProperty(parts[0]) && !parts[0] == 'defaults') {
	...
	if (parts[0] == 'defaults') {
		...
		if (parts.length > 1) return reject(ERROR.PACKAGE_PROPERTY_LENGTH, parts.join('.'));
		return this.package.set(parts[0], value, array, null, remove);
	} else {
...
proto.ensureUserTemplates = function () {
	var enyo = this;
	return fs.ensureDirAsync(u_templates).then(function () {
		return enyo.loadUserTemplates();
	});
};

proto.loadUserTemplates = function () {
	var enyo = this;
	return fs.readdirAsync(u_templates).then(function (files) {
		enyo.user.templates = {};
		return Promise.map(files, function (file) {
			file = path.join(u_templates, file);
			return enyo.loadTemplate(file).then(function (data) {
				enyo.user.templates[data.name] = data;
			});
		});
	}).then(function () {
		return enyo.loadDefaultTemplates();
	});
};

proto.loadDefaultTemplates = function () {
	var enyo = this;
	enyo.system.templates = {};
	return Promise.map([
		path.join(__dirname, 'default-templates', 'default-library'),
		path.join(__dirname, 'default-templates', 'default-app')
	], function (target) {
		return enyo.loadTemplate(target).then(function (data) {
			enyo.system.templates[data.name] = data;
		});
	});
};

proto.loadTemplate = function loadTemplate (target) {
	var
		ret = {path: target},
		config = path.join(target, '.enyoconfig'),
		pkg = path.join(target, 'package.json');
	return Promise.all([
		fs.readJsonAsync(config).then(null, function () { return {}; }).then(function (data) {
			ret.config = data;
		}),
		fs.readJsonAsync(pkg).then(null, function () { return {}; }).then(function (data) {
			ret.package = data;
		})
	]).then(function () {
		ret.name = ret.config.name || ret.package.name || path.basename(target);
		// if (Object.keys(ret.config).length === 0) cli('The template "%s" (%s) does not have a valid configuration file', ret.name, ret.path);
		// if (Object.keys(ret.package).length === 0) cli('The template "%s" (%s) does not have a valid package.json file', ret.name, ret.path);
		return ret;
	});
}