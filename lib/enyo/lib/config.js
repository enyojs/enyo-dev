...
var exports = module.exports = Promise.method(function (opts) {
	console.log(opts.env ? 'has env' : 'does not have env');
	if (opts.env) {
		cli(opts.env.hasPackage());
		cli(opts.env.hasConfig());
		cli(opts.env.get('libraries'));
		cli(opts.env.get('libDir'));
		cli(opts.env.get('interactive'));
		cli(opts.env.get('targets.enyo'));
	}
});
...
// 			return opts.global ? initFileExists(file, opts) : Promise.resolve();
...
// 	} else if (opts.env.get('useDefaultConfig')) {
// 		return fs.writeJsonAsync(file, clone(opts.env.defaultConfig.projectDefaults), {spaces: 2});
// 	} else if (!opts.env.get('interactive')) {
// 		if (opts.env.hasGlobalConfig()) {
//
// 		}
// 	} else {
