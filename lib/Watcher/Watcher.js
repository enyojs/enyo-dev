...
function Watcher (params) {
	if (!(this instanceof Watcher)) return new Watcher(params);
	...
	if (opts.production) throw new Error(
...
Watcher.prototype._initPackager = function (done) {
	...
	var
		watcher = this,
		opts = this.options,
		pack = this._packager = Packager({opts: opts, env: opts.env});
		...
		if (!entry.external) {
			if (entry.isPackage) {
				...
				if (entry.styles && entry.styles.length) {
					entry.styles.forEach(function (se) {
						var dir = path.dirname(se);
...
Watcher.prototype._processAsset = function (file) {
	...
	for (var i = 0; i < cache.length && !found; ++i) {
		...
		if (entry.isPackage) {
			if (entry.assets && entry.assets.length) {
				if (entry.assets.indexOf(file) > -1) {
					found = true;
					idx = i;
					break;
				}
...
Watcher.prototype._processStyle = function (file) {
	...
	for (var i = 0; i < cache.length && !found; ++i) {
		entry = cache[i];
		if (entry.isPackage && entry.styles && entry.styles.length) {
			if (entry.styles.indexOf(file) > -1) {
				found = true;
				idx = i;
				break;
			}
