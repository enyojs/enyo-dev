...
* can find it easily.
*/
function translateImportPaths (text, base) {
	text = text.replace(/(\@import\s+(['"])(?!https?)([a-zA-Z0-9\ \/\-\.\@\{\}]+)\2)/g,
		function (match, full, wrap, src) {
			...
			if (src.charAt(0) != '/') {
				...
				logger.log('debug', 'translating @import from "%s" to "%s"', match, ret);
				return ret;
			} else return full;
		}
	);
...
function translateUrlPaths (text, base, origin, pkg) {
	text = text.replace(/url\((?!http)(?:\'|\")?([a-zA-Z0-9\ \.\/\-]*)(?:\'|\")?\)/g,
		function (match, exact) {
			...
			if (exact.charAt(0) != '/') {
				...
				logger.log('debug', 'translating URL from "%s" to "%s"', match, ret);
				return ret;	
			} else return match;
		}
	);
