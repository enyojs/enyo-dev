...
module.exports = Promise.method(function (opts) {
	...
	// we test now to determine if the target package even exists to prevent it from being
	// created unnecessarily during the env initialization routine
	return fs.lstatAsync(opts.package).then(function (stat) {
		if (stat.isDirectory()) {
		...
		utils.fatal('%s is not a valid package', opts.package);
	});
});
...
function configure (opts) {
	...
	opts.name = string('name', opts.name, opts) || path.basename(opts.package);
	...
	// cli - relative to cwd, otherwise, package
	opts.headScripts = clipaths('headScripts', opts.headScripts, opts);
	// cli - relative to cwd, otherwise, package
	opts.tailScripts = clipaths('tailScripts', opts.tailScripts, opts);
	opts.promisePolyfill = bool('promisePolyfill', opts.promisePolyfill, opts);
	opts.styleOnly = bool('styleOnly', opts.styleOnly, opts);
	opts.outCssFile = outCssFile(opts);
	opts.lessVars = lessVars(opts.lessVars, opts);
...
function outCssFile (opts) {
	if (opts.styleOnly && opts.library) {
		return typeof opts.outCssFile == 'string' ? opts.outCssFile : null;
	} else {
		return string('outCssFile', opts.outCssFile, opts);
	}
}
...
function lessVars (cli, opts) {
	var ret = cli && Array.isArray(cli) ? cli : [];
	ret = ret.concat(defaults('lessVars', opts));
	return ret.filter(function (e) {
		return typeof e == 'object' && e.name && e.hasOwnProperty('value');
	});
}

function moduleDir (opts) {
	return path.join(opts.package, opts.env.package.get('moduleDir'));
}