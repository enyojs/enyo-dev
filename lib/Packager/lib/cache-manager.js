...
var
	...
	resetKeys = ['request', 'dependents', 'dependencies', 'bundle', 'bundleName', 'origins', 'rawContents', 'styleEntries', 'assetEntries', 'trace'];
...
CacheStream.prototype._flush = function (done) {
	...
	if (opts.cache) {
		...
		writeCache(opts.cacheFile, cache).then(function () {
			...
			utils.fatal('could not write cache file %s:\n\t%s', opts.cacheFile, e.message);
		});
