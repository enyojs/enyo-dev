...
var
	...
	utils = require('../../../utils'),
	fatal = require('../../../logger').fatal;

var
	getResolver = require('../../module-resolver').default;
...
function ProcessSourceStream (opts) {
	...
	opts = opts || {};
	log = logger(opts).child({component: 'process-source-stream'});
	this.options = opts;
	
	log.level(opts.logLevel);

	this.preprocessors = opts.preprocessors = defined(opts.preprocessors, []);
	this.resolver = defined(opts.resolver, getResolver());
...
ProcessSourceStream.prototype._resolve = function (entry) {
	...
	log.info({module: entry.relName, base: entry.base}, 'resolving');
	...
	var result = resolver({opts: opts, target: entry.path, cwd: entry.base || opts.cwd, paths: opts.paths});
	if (result === false) {
		this._resolved(new Error('Could not resolve module "' + entry.relName + '"'), entry);
	} else {
		merge(entry, result);
		entry.relPath = entry.external ? entry.relName : (path.relative(opts.cwd, entry.fullpath) || entry.fullpath);
		if (entry.lib && entry.libName) {
			this.libs[entry.libName] = entry.lib;
		}
		this._resolved(null, entry);
	}
...
ProcessSourceStream.prototype._resolved = function (err, entry) {
	...
	if (err) {
		...
		log.trace({module: entry.relName}, 'Could not find module', err);
		log.error({module: entry.relName}, 'could not be resolved, will be reported at the end');
...
ProcessSourceStream.prototype._finalize = function () {
	...
	if (errored.length) fatal('Cannot continue due to encountered errors');
