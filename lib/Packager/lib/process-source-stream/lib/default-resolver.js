...
function loadDirectory (x, module) {
	...
	if (stat && stat.isDirectory()) {
		...
		try {
			...
			log.debug({module: module}, 'path %s is a directory, but not a package');
		...
		if (dirs.length) {
			...
			for (i = 0; i < dirs.length; ++i) {
				...
				if (prev && prev.isPackage && prev.json.moduleDir) {
					search = path.join(search, prev.json.moduleDir);
				} else {
				...
				if (!res) {
					...
					if (!prev || (prev.isPackage && !prev.json.moduleDir)) {
						...
						search = path.join(last, 'lib', dir);
						
						log.debug({module: module},
							'unable to find step without moduleDir, attempting default "lib": %s',
							search
						);
			...
			if (search) {
				...
				if (!res) {
					...
					if (!res) {
						search = path.join(last, 'lib', base);
						res = checkExternal(search, module);
					}
