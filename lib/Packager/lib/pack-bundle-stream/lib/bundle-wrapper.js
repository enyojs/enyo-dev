	...
	if (!require || typeof require != 'function') {
		require = scope.require = function (target) {
			...
			mod(
				...
				!request ? null : function (name) {
					return request(map && map.hasOwnProperty(name) ? map[name] : name);
				}
			);
	...
	if (require.manifest) {
		Object.keys(manifest).forEach(function (key) {
			...
			if ((existing = require.manifest[key])) {
				...
				/*debug*/console.warn('overriding existing manifest entry %s', key, existing, value);
			}
