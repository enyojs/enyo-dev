...
proto.pack = function () {
	var packer, bundler, bundle, manifest, opts;
	packer = this;
	...
	bundle.order.forEach(function (name) {
		...
		// we should not include modules from skipped libraries and the reason we have to check
		// for this is because they may have been stuffed into some other bundle depending on the
		// factoring scheme used
		if (opts.skip && entry.libName && opts.skip.indexOf(entry.libName) > -1) return;
