...
function WriteFilesStream (opts) {
	...
	opts = opts || {};
	log = logger(opts).child({component: 'write-files-stream'});
	this.count = 0;
	this.skipped = 0;
	this.options = opts;
	this.cleaned = false;
...
proto._write = function (file, nil, done) {
	...
	return Promise.resolve().then(function () {
		...
		if(file.copy && copyMap[file.source] && copyMap[file.source]===file.outfile) {
			stream.skipped++;
		} else {
...
proto.ensureOutputDirectory = function () {
	...
	return fs.ensureDirAsync(opts.outDir).then(function () {
		...
		utils.fatal('could not ensure the output directory %s:\n\t%s', stream.relative(opts.outDir), e.stack);
	});
};
...
proto.cleanOutputDirectory = function () {
	...
	return fs.emptyDirAsync(opts.outDir).then(function () {
		...
		utils.fatal('could not clean the output directory %s:\n\t%s', stream.relative(opts.outDir), e.stack);
	});
};
