...
function ProcessStyleStream (opts) {
	...
	opts = opts || {};
	log = logger(opts).child({component: 'process-style-stream'});
	this.options = opts;
	this.bundles = [];
	this.sources = {};
	this.lessOnlyLess = !! opts.lessOnlyLess;
	this.lessVars = opts.lessVars || [];
...
proto.precompile = Promise.method(function (files) {
	...
	if (this.lessVars.length) {
		source += this.lessVars.map(function (e) {
			var variable, value;
			variable = e.name;
			value = e.value;
...
proto.postprocess = Promise.method(function (entries) {
	...
	entries.forEach(function (entry) {
		...
		if (entry.contents) {
			...
			log.debug(`Updating style for bundle "${bundle.name}"`);
		}
	});
...
proto.resolve = Promise.method(function (bundle) {
	...
	if (!order || !Array.isArray(order) || !modules) return [];
...
function translateUrlPaths (contents, base, file, entry, bundle, opts) {
	...
	return contents.replace(/url\((?!(['"])?((http|data)\S+))(['"]?)(\S+?)\4\)/g, function (match, n0, n1, n2, n3, sub) {
		if (!utils.isAbsolute(sub)) {
			...
			if (file.lastIndexOf('.less')===file.length-5) {
				actual = actual.replace(/%7B/g, '{').replace(/%7D/g, '}');
			}
			ret = 'url(\'' + actual + '\')';
