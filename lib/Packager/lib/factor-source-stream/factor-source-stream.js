...
proto.bundle = function (entry) {
	...
	if (entry.trace) {
		if (log.debug()) log.debug({bundle: bundle.name, module: entry.relName}, 'setting bundle.request to %s from %s', !! entry.trace.request, !! bundle.request);
		bundle.request = !! entry.trace.request;
		bundle.includeRequest = !! entry.requests.length;
	}
...
proto.trace = function (entry) {
	...
	entry.dependents.forEach(function (name) {
		...
		if (o_r && o_rs) utils.streamError(stream, 'module %s is required and requested by module %s', entry.relName, dep.relName);
	});
	...
	if (requested && !required) {
		...
		if (log.debug()) log.debug({module: entry.relName, trace: trace, dependentRequested: o_requested, dependentRequired: o_required, requested: requested, required: required});
		...
		if (o_required && !o_requested) {
			...
			if (requires.length === 1) {
				...
				entry.bundleName = stream.customNameFor(entry);
				trace.requested = true;
				if (db) log.debug({module: entry.relName, bundle: entry.bundleName}, 'module is required by dynamically requested modules and will be a shared dependency of those modules');
			}
		} else {
			entry.bundleName = stream.getBundleNameFor(entry);
			trace.required = true;
			// this is marked because we need to ensure that the request API is available in the
			// bundle where the module belongs
			trace.requested = true;
