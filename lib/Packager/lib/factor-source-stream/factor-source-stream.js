...
proto.bundle = function (entry) {
	...
	if (entry.trace) {
		if (log.debug()) log.debug({bundle: bundle.name, module: entry.relName}, 'setting bundle.request to %s from %s', !! entry.trace.requested, !! bundle.request);
		bundle.request = !! entry.trace.requested;
