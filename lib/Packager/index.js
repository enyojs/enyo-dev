...
Packager.prototype.addPackage = function (pkg) {
	if (this.packages.order.indexOf(pkg.__dirname) === -1) {
		...
		logger.log('debug', 'added package from "%s"', path.relative(this.package, pkg.__dirname) || './');
	}
};
...
Packager.prototype.processProjectDependencies = function () {
	...
	logger.log('debug', 'included libraries', this.includeLibs);
	logger.log('debug', 'current known packages', this.packages.order.map(function (entry) {
		return path.relative(this.package, entry) || './';
	}, this));
...
Packager.prototype.processIncludedLibraries = function () {
	...
	logger.log('debug', 'processing included libraries');
	...
	this.includeLibs.reverse().forEach(function (lib) {
		
		// determine if the library has a package then add it
		var
			libPath = path.resolve(this.package, this.libPath, lib),
			packagePath = path.join(libPath, 'package.json'), pkg;
		...
		try {
			...
			pkg.__dirname = libPath;
		} catch (e) {
			// we don't actually care about the fact it wasn't there but will notify anyway
			// this is not a critical error
			logger.log('warn', 'library "%s" did not have a package.json package file', path.join(this.libPath, lib));
		}
