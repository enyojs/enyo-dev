...
validJsonKeys.push('optionalLibs');
...
function Packager (opts) {
	...
	this.optionalLibs = this.optionalLibs || [];
...
Packager.prototype.development = function () {
	...
	// check known libs for optional dependencies to be ignored
	this.processOptionalLibs(bundle);
...
/**
* Check to see if the known libraries include a package.json object to analyze for optional
* dependencies
*/
Packager.prototype.processOptionalLibs = function (bundle) {

	logger.log('info', 'processing known libraries for optional package dependencies');
	logger.log('debug', this.knownLibs);

	// load the packages of all knownLibs
	this.loadPackages(this.knownLibs, function (pkg) {
		// and if they have optionalLibs, add them to our list
		if (pkg.optionalLibs) this.optionalLibs.push.apply(this.optionalLibs, pkg.optionalLibs);
	});

	logger.log('debug', 'Found %s optional libs', this.optionalLibs.length);
	this.optionalLibs.forEach(function (name) {
		if (!this.hasLib(name)) {
			logger.log('info', 'Optional library %s not found in libPath. Ignoring in build.', name);
			bundle.ignore(name);
		}
	}, this);
};
...
Packager.prototype.processIncludedLibraries = function () {
	...
	this.loadPackages(this.includeLibs, function (pkg) {
		this.addPackage(pkg, true);
	});
};

Packager.prototype.loadPackages = function (libs, cb) {
...
Packager.prototype.processStyle = function () {
	...
	styles.forEach(function (entry) {
		...
		files.forEach(function (file) {
			...
			if (src) {
				...
				src = utils.translateUrlPaths(src, base, path.dirname(file), entry.package, file, assets);
				output += src + '\n';
			}
		}, this);
	...
	// assign this for later evaluation alongside the requested assets for comparative purposes
