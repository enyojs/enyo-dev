...
function Packager (opts) {
	...
	logger.setEnabled(opts.verbose);
	
	logger('Packager(): Package location: ' + this.package);
...
Packager.prototype.prepareTemplate = function () {
	...
	if (tpl) {
		...
		logger('Packager.prepareTemplate(): Using provided template index file: ' + tpl);
		...
		logger('Packager.prepareTemplate(): Using default template index file');
...
Packager.prototype.preparePackageJson = function () {
	...
	if (!json.main) {
		...
		logger('Packager.preparePackageJson(): No "main" provided, using default "index.js"');
		
		json.main = path.resolve(this.package, 'index.js');
	}
	else {
		logger('Packager.preparePackageJson(): Using provided "main": ' + json.main);
...
Packager.prototype.addLib = function (lib) {
	if (this.libs.indexOf(lib) === -1) {
		this.libs.push(lib);
		logger('Packager.addLib(): Adding library ' + lib + ' -> [' + this.libs.join(', ') + ']');
		this.emit('lib', lib);
	}
};
