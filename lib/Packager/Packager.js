...
var
	...
	analytics = require('./lib/analytics-stream'),
...
proto.buildPackage = function (opts) {
	...
	if (opts.listOnly) {
		...
		stream
			.pipe(sort(opts))
			.pipe(factor(opts))
			// .pipe(require('through2').obj(function (bundle, nil, next) {
			// 	console.log(bundle.name);
			// 	var a = [];
			// 	a.push(bundle.name);
			// 	a = a.concat(bundle.order);
			// 	fs.writeJsonAsync(bundle.name + '.json', a, {spaces: 2}).then(function () {
			// 		next(null, bundle);
			// 	});
			// }))
			.pipe(sort(opts))
